...
# for an Adept Smart Controller CS robots using Adept's V+ programming language
#
...
def pose_2_str(pose,joints=None,reference=None):
    ...
    return ('TRANS(%.3f,%.3f,%.3f,%.3f,%.3f,%.3f)' % (x,y,z,w,p,r))
...
class RobotPost(object):
    """Robot post object"""
    PROG_EXT = 'vb1'        # set the program extension
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        for i in range(len(joints)):
            self.addline('DRIVE %i, %.3f, %.1f' % (i+1, joints[i], self.SPEED_PERCENT))
        if pose is not None:
            self.addline('MOVE frame1:' + pose_2_str(pose,joints))
        
    def MoveL(self, pose, joints, conf_RLF=None):
        """Add a linear movement"""
        if pose is None:
            self.setFrame(eye(4))
        self.addline('MOVES frame1:' + pose_2_str(pose,joints))
    ...
    def setFrame(self, pose, frame_id=None, frame_name=None):
        ...
        self.REF_FRAME = pose
        self.addline('SET frame1 = ' + pose_2_str(pose))
        # Adept's V+ for Staubli does not support changing the reference frame using complete orientation definition. We must then shift all targets and use the robot reference frame as the active reference frame        
    ...
    def setSpeed(self, speed_mms):
        ...
        # assume 4000 mm/s to be 100% of the speed
        self.SPEED_PERCENT = min(100*speed_mms/4000, 100)
        self.addline('SPEED %.1f, 100 MMPS ALWAYS' % (speed_mms))
        #self.addline('SPEED %.1f, %.1f MMPS ALWAYS' % (speed_mms, seld.SPEED_PERCENT))
