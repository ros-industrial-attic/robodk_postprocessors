...
class RobotPost(object):
    """Robot post object"""
    BASE_PROGNAME = 'SRA120EL-01-A'
    MAX_LINES_X_PROG = 950 # maximum number of lines per program
    PROG_ID = 123          # Program ID to store the program
    nPROGS = 0
    ...
    def ProgSave(self, folder, progname, ask_user = False, show_result = False):
        ...
        if ask_user or not DirExists(folder):
            ...
            filesave = '%s/%s.%i' % (folder , progname , self.PROG_ID)
        #----------------------------
        # Save file(s)
        if self.nPROGS > 0: # save multiple programs
            self.PROG_FILES = []
            ...
            for i in range(len(self.PROGS)):
                fsavei = ('%s/%s.%i' % (folder, progname_base, self.PROG_ID+i+1))
                mainprog = mainprog + ('%s.%i\n' % (progname_base, self.PROG_ID+i+1))
                fid = open(fsavei, "w")
            ...
            filesave = '%s.%i' % (filesave, self.PROG_ID)
            fid = open(filesave, "w")
            fid.write(self.PROG)
            fid.close()
            print('SAVED: %s\n' % filesave) # tell RoboDK the path of the saved file
    ...
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        ...
        self.addline('MOVEX M1X,C1,%s,S=%.2f,H=%i,MS' % (pose_2_str(pose1,joints1,self.REF_FRAME) , self.SPEED_MMS , self.TOOL_ID))
        self.addline('MOVEX M1X,C2,%s,S=%.2f,H=%i,MS' % (pose_2_str(pose2,joints2,self.REF_FRAME) , self.SPEED_MMS , self.TOOL_ID))
...
# ------------------ private ----------------------                
    def addline(self, newline):
        ...
        if self.CURRENT_LINES > self.MAX_LINES_X_PROG:
            ...
            self.PROGS.append(self.PROG)
            self.nPROGS = self.nPROGS + 1
            self.ProgStart(self.PROGRAM_NAME)
        #-----------------------
