...
def pose_2_str(pose):
    ...
    c = angle3(pose.VX(),[1,0,0])*180/pi # angle between X axes
    return ('TRANS(%.3f,%.3f,%.3f,%.3f)' % (x,y,z,c))
...
def target_2_str(pose,joints):
    ...
    if joints[1] >= 0:
        ...
        config = 2
    return ('POINT(%.3f,%.3f,%.3f,%.3f, %.1f, %i)' % (x,y,z,c,t,config))
...
class RobotPost(object):
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        self.addline('MOVE ' + target_2_str(pose, joints))
        
    def MoveL(self, pose, joints, conf_RLF=None):
        """Add a linear movement"""
        self.addline('MOVES ' + target_2_str(pose, joints))
        
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        """Add a circular movement"""
        self.addline('MOVEC %s %s' % (target_2_str(pose1, joints1), target_2_str(pose2, joints2)))
    ...
    def setSpeed(self, speed_mms):
        ...
        speed_percent = max(100*speed_mms/5000,100)
        self.addline('SPEED=%.0f' % (speed_percent))
