...
def pose_2_str(pose, ref_frame):
    ...
    [x,y,z,r,p,w] = pose_2_xyzrpw(ref_frame*pose)
    return ('%.3f,%.3f,%.3f,%.3f,%.3f' % (x,y,z,p,w))
...
class RobotPost(object):
    ...
    def MoveJ(self, pose, joints, conf_RLF=None):
        ...
        self.POSE_LAST = pose
        self.addline('MJ %s' % joints_2_str(joints))
        
    def MoveL(self, pose, joints, conf_RLF=None):
        """Add a linear movement"""
        self.POSE_LAST = pose
        self.addline('MP %s' % pose_2_str(pose,self.REF_FRAME))
        
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        """Add a circular movement"""
        self.addlog('Circular move not possible')
        
    def setFrame(self, pose, frame_id=None, frame_name=None):
        """Change the robot reference frame"""
        self.REF_FRAME = pose
        #elf.addline('Base %s' % pose_2_str(pose))
        
    def setTool(self, pose, tool_id=None, tool_name=None):
        """Change the robot TCP"""
        self.addline('TL %.0f' % pose.Pos()[2])
    ...
    def Pause(self, time_ms):
        ...
        if time_ms < 0:
            ...
            self.addline('TI %.0f' % (time_ms*0.01))
    
    def setSpeed(self, speed_mms):
        """Changes the robot speed (in mm/s)"""
        # Assume 5000 mm/s as 9/9
        speed_percent = min(speed_mms*9/5000, 9)
        self.addline('SP %.0f' % speed_percent)
    ...
    def setZoneData(self, zone_mm):
        ...
        self.addlog('Rounding not possible')
