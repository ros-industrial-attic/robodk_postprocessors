...
class RobotPost(object):
    ...
    MAX_LINES_X_PROG = 2500  # maximum number of lines per program. It will then generate multiple "pages (files)"
    ...
    def ProgSave(self, folder, progname, ask_user = False, show_result = False):
        if len(self.PROG_LIST) >= 1:
            ...
            for i in range(npages):
                ...
                self.progsave(folder_user, self.PROG_NAMES[i], ask_user, show_result)
                if i == 0:
                    ...
                    self.LOG = ''
                    ask_user = False # ask only for the first program
                    show_result = False # only show the first program, if required
