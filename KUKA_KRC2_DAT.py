...
class RobotPost(object):
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        wid = ''
        if not self.ARC_ON:
            self.addline(';FOLD LIN P%s %sVel=%.0f m/s CPDAT%s Tool[%i] Base[%i];%%{PE}%%R 5.5.0,%%MKUKATPA20,%%CARC_SWI,%%VLIN,%%P 1:LIN, 2:P%s, 3:%s, 5:%.0f, 7:CPDAT%s' % (vname, str_cont, self.speed_ms, vname, self.TOOL_ID, self.BASE_ID, vname, str_cdis, self.speed_ms, vname))
        else:
